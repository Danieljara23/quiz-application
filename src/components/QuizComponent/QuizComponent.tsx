...
interface Props {
  ...
  answers: [any];
  onAnswerSelected: (event: Event, isCorrect: boolean) => void;
  onNextQuestion: () => void;
  userAnswers: [any];
  setUserAnswers: (isCorrect: boolean) => void;
}

function QuizComponent(props: Props): JSX.Element {
  const {
    counter,
    total,
    questionTitle,
  ...
  return (
    <div css={quizContainerCss}>
      {!showFinalResult ? (
          ...
          <div css={answerListContainerCss}>
            ...
            <ul css={answerListCss}>
              {answers.map((item, index) => {
                let type;
                if (
                  (showCorrect === item.id && item.isCorrect) ||
                ...
                const contentLengthType =
                  item.description.length > 39 ? 'large' : 'default';
        ...
        <FinalResult userAnswers={userAnswers} />
      )}
      {!showFinalResult && (
        <div css={controlsCss}>
          <button css={arrowCss} onClick={handleNextQuestion}></button>
        </div>
      )}
    </div>
  );
