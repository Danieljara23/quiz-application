...
function useCreateQuestionnaire() {
  ...
  const [createQuestionnaireMutation, { data, error, loading }] = useMutation(
  ...
  const addOption = useCallback((questionId) => {
    setQuestionnaireState((prevState) => ({
      ...prevState,
      questions: [
        ...prevState.questions.map((item, index) =>
          index === questionId
            ? {
                ...item,
                answers: [
                  ...item.answers,
                  {
                    description: `Opción ${item.answers.length + 1}`,
                    isCorrect: false,
                  },
                ],
              }
  ...
  const removeQuestionOption = useCallback((optionId, questionId) => {
    setQuestionnaireState((prevState) => ({
      ...prevState,
      questions: [
        ...prevState.questions.map((question, index) =>
          index === questionId
            ? {
                ...question,
                answers: [
                  ...question.answers.filter((answer, idx) => idx !== optionId),
                ],
              }
            : question
        ),
  ...
  const createQuestionnaire = useCallback(() => {
    ...
    questionnaireState.questions.map(
      (question) => delete question['enableEdition']
    );
